...
SoundFile is designed to be efficient for common I/O tasks, but audio can be large. This page outlines the main levers you
have.
...
`sf_read()` reads data in a loop using `blocksize` frames per iteration, but still allocates a single output buffer for
